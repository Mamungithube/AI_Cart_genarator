...
def get_active_gemini_key() -> str:
    """
    Returns the active Gemini API key from ai_config.json, settings, or environment variables.
    """
    ...
    if Path(config_file).exists():
        try:
            with open(config_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
                key = data.get('gemini_key') or data.get('api_key') or ''
                if key and (key.startswith('AIza') or key.startswith('AQ.') or data.get('provider') == 'gemini'):
                    return key.strip()
